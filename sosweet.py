...
def server1(i, name):
    ...
    if match:
        ...
        with open("doc/playlist1.m3u8", "a") as file:
            file.write(f"#EXTINF:-1 tvg-logo="https://raw.githubusercontent.com/InfinityBohr/SweetSoup/refs/heads/main/doc/images/pinkerotic.png" group-title="Hot and Spicy" ,{name}\n")
...
def server2(hash, name):
    ...
    with open("doc/playlist2.m3u8", "a") as file:
...
def server3(hash, name):
    ...
    stream_url = f"https://moonlight.wideiptv.top/{name}/index.fmp4.m3u8?token={token}"
    with open("doc/playlist3.m3u8", "a") as file:
        file.write(f"#EXTINF:-1,{name}\n")
        file.write(f"{stream_url}\n")
